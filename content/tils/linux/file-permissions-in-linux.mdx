...
While [connecting to a remote server using SSH](http://localhost:3000/tils/aws/how-to-ssh-into-aws-ec2), the first step was to change the permissions of the key-pair file using the command : 
...
As there are 3 permissions for 3 types of users, we can allocated 3 cells to each users. Each cell can have a value 'r', 'w', 'x' or '-' if the user has no permissions. 
